...
router.get('/twitter', protect, asyncHandler(async (req, res, next) => {
  try {
    ...
    // For production, use Redis or a secure session store
    // For now, we'll send them to the client to include in callback
    res.status(200).json({
      status: 'success',
      data: {
        authUrl: url,
        state,
        codeVerifier, // In production, store this server-side
      }
    });
  } catch (error) {
...
router.get('/twitter/callback', asyncHandler(async (req, res, next) => {
  const { code, state, codeVerifier, userId } = req.query;
